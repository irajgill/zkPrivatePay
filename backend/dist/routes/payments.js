...
router.post('/confidential', async (req, res) => {
    try {
        const aptosAddress = req.headers['x-aptos-address'];
        if (!aptosAddress) {
            return res.status(401).json({ error: 'Aptos address required' });
        }
        const { proofJobId, attesters, signatures, nullifiers, commitments, feePaid, newStateRoot } = ConfidentialPaymentSchema.parse(req.body);
        ...
        const paymentId = await Database.createPayment(user.id, nullifiers, commitments, BigInt(feePaid));
        res.json({
            paymentId,
            payload,
...
router.post('/withdraw', async (req, res) => {
    try {
        ...
        const { proofJobId, attesters, signatures, recipient, amount } = WithdrawSchema.parse(req.body);
        const proofJob = await Database.getProofJob(proofJobId);
        if (!proofJob || proofJob.status !== 'completed') {
            return res.status(400).json({ error: 'Invalid or incomplete proof job' });
        }
